...
async fn notify_progress(
    listener: &tokio::sync::RwLock<Option<Arc<dyn TransferProgressListener>>>,
    upload: bool,
    p: &ProgressPayload,
) {
...
pub async fn cmd_upload_file(
    ...
    // Emit start progress
    if !tid.is_empty() {
        notify_progress(&listener_lock, true, &ProgressPayload {
            id: tid.clone(), percent: 0, transferred_bytes: 0, total_bytes: size, speed_bytes_per_sec: 0,
        }).await;
    }
    ...
    let progress_listener_lock = listener_lock.clone();
    let progress_task = if !tid.is_empty() {
        Some(tokio::spawn(async move {
            ...
            loop {
                ...
                notify_progress(&progress_listener_lock, true, &ProgressPayload {
                    id: progress_tid.clone(), percent, transferred_bytes: current, total_bytes: file_size, speed_bytes_per_sec: speed,
                }).await;
    ...
    if !tid.is_empty() {
        notify_progress(&listener_lock, true, &ProgressPayload {
            id: tid, percent: 100, transferred_bytes: size, total_bytes: size, speed_bytes_per_sec: 0,
        }).await;
    }
...
pub async fn cmd_download_file(
    ...
    bw.can_transfer(total_size)?;

    // Emit start
    if !tid.is_empty() {
        notify_progress(&listener_lock, false, &ProgressPayload {
            id: tid.clone(), percent: 0, transferred_bytes: 0, total_bytes: total_size, speed_bytes_per_sec: 0,
        }).await;
    }
    ...
    while let Some(chunk) = download_iter.next().await.transpose() {
        ...
        if !tid.is_empty() {
            ...
            if dt >= 0.25 || downloaded >= total_size {
                ...
                notify_progress(&listener_lock, false, &ProgressPayload {
                    id: tid.clone(), percent, transferred_bytes: downloaded, total_bytes: total_size, speed_bytes_per_sec: speed,
                }).await;
                last_emit_time = now;
    ...
    bw.add_down(total_size);

    // Emit completion
    if !tid.is_empty() {
        notify_progress(&listener_lock, false, &ProgressPayload {
            id: tid, percent: 100, transferred_bytes: downloaded, total_bytes: total_size, speed_bytes_per_sec: 0,
        }).await;
    }
