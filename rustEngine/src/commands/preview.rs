...
use crate::commands::utils::{resolve_peer, data_dir, cache_dir};
...
pub async fn cmd_get_preview(
    ...
    let cache_dir = cache_dir(state)?.join("previews");
    if !cache_dir.exists() {
    ...
    if let Some(msg) = target_message {
        if let Some(media) = msg.media() {
            ...
            let file_ready = if save_path.exists() {
                ...
                if let Err(e) = bw.can_transfer(size) {
...
pub async fn cmd_get_thumbnail(
    ...
    let cache_dir = data_dir(state)?.join("thumbnails");
    if !cache_dir.exists() {
        let _ = std::fs::create_dir_all(&cache_dir);
    }
